...
def title_case_structure(name: str) -> str:
    """
    Convert a structure name to a consistent display-friendly title case.

    Rules
    -----
    - Parenthetical abbreviations are upper-cased, e.g. "(csp)" → "(CSP)".
    - Short conjunctions / prepositions (of, the, and …) stay lower-case
      **unless** they are the first word.
    - Every other word is capitalised normally.
    - Numbers attached to a word keep their original position, e.g. "1" stays.

    Examples
    --------
    "thalamus 1"                         → "Thalamus 1"
    "csp (cavum septum pellucidum)"      → "CSP (Cavum Septum Pellucidum)"
    "anterior horn of lateral ventricles 2"
                                         → "Anterior Horn of Lateral Ventricles 2"
    "petrous part of temporal bone 1"    → "Petrous Part of Temporal Bone 1"
    """
    import re

    # Handle optional leading abbreviation token like "csp" before a parenthesis
    # e.g. "csp (cavum septum pellucidum)" → first token is the abbreviation
...
def _load_dicom_as_gray_uint8(path: str) -> np.ndarray:
    """
    Read a DICOM file and return a 2-D uint8 grayscale numpy array.
    Handles RGB/YBR, MONOCHROME1 inversion, multi-frame, and single-frame
    DICOMs with a leading size-1 dimension.
    """
    ...
    if arr.ndim == 3:
        if arr.shape[-1] in (3, 4):
            # H×W×C  (RGB / YBR / RGBA) → weighted luminance
            arr = (0.2989 * arr[..., 0]
                 + 0.5870 * arr[..., 1]
                 + 0.1140 * arr[..., 2])
        elif arr.shape[0] == 1:
            # 1×H×W  (single-frame with leading dim)
            arr = arr[0]
        else:
            # N×H×W multi-frame: take the middle frame
    ...
    return arr.astype(np.uint8)   # guaranteed 2-D
...
def preprocess_from_array(gray: np.ndarray, H: int, W: int) -> torch.Tensor:
    ...
    # Ensure truly 2-D before building tensor (defensive guard)
    if gray.ndim == 3:
...
def run_inference(
    ...
    for img_path in img_paths:
        ...
        if progress_callback is not None:
            ...
            # print(f" Image: {img_path_str} ")
            # print(f"img_path : {img_path} ")
            progress_callback(img_path_str, is_stnd)

        # print(f"    [PLANE]     {plane_info['predicted_plane']}  "
        #       f"status={plane_info['plane_standard']}  "
        #       f"match={plane_info['plane_match_frac']:.0%}  "
        #       f"conf={plane_info['plane_mean_conf']:.3f}")
        # print(f"    [LOCAL-VIS] overall={struct_summary['vis_overall_flag']}  "
        #       f"ok={struct_summary['vis_n_structures_ok']}/"
        #       f"{struct_summary['vis_n_structures_predicted']}  "
        #       f"low_contrast={struct_summary['vis_n_structures_low_contrast']}  "
        #       f"low_conf={struct_summary['vis_n_structures_low_conf']}  "
        #       f"blurry={struct_summary['vis_n_structures_locally_blurry']}")
        # for r in struct_results:
        #     if r.get("predicted"):
        #         name = r["name"]
        #         print(f"      {name:<45} {r['vis_flag']:<20} "
        #               f"contrast={r.get('local_contrast', 0):.1f} ({_qual_contrast(r.get('local_contrast', 0), name)})  "
        #               f"blur={r.get('local_blur', 0):.1f} ({_qual_blur(r.get('local_blur', 0), name)})  "
        #               f"conf={r.get('model_confidence', 0):.3f} ({_qual_conf(r.get('model_confidence', 0), name)})")
    ...
    # print(f"      └─ inference_summary.csv")
    # print(f"      └─ structure_details.json")
