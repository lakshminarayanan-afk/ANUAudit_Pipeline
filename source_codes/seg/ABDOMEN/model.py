...
# #             nn.Conv2d(f, Config.NUM_CHANNELS, kernel_size=1),
# #             # No activation here — raw logits, sigmoid applied in loss/inference
# #         )
...
# #         """
# #         Returns:
# #             (B, NUM_CHANNELS, H, W)  raw logits
# #             channels 0–14: structures,  channel 15: skin line
# #         """
# #         x1 = self.inc(x)
# #         x2 = self.down1(x1)
# #         x3 = self.down2(x2)
...
# #     m   = build_model()
# #     out = m(torch.randn(2, 1, 512, 512))
# #     print(f"Output: {out.shape}")   # (2, 16, 512, 512)
...
# """
# model.py  –  UNet with ASPP bottleneck for fetal abdomen segmentation
# ======================================================================
# Changes vs. original:
#   • ASPP (Atrous Spatial Pyramid Pooling) at the bottleneck captures
#     multi-scale context — helps small / thin structures (Renal Pelvis,
#     Cord Insertion, Adrenals, TK plane kidneys).
#   • Deeper classification head (3×3 → BN → ReLU → 1×1) with a separate
#     per-structure depth before the final projection.
#   • Output: (B, NUM_CHANNELS, H, W) raw logits; NUM_CHANNELS = 14.
# """
...
# #  Replaces the flat bottleneck with four parallel dilated convolutions
# #  + global average pooling branch, fused by a 1×1 projection.
# # ─────────────────────────────────────────────────────────────────────────────
...
#     """
#     Receives the bottleneck feature map and returns a feature map of the
#     same spatial resolution with richer multi-scale context.

#     Branches: 1×1 conv, 3×3 dilated (r=6,12,18), global avg-pool.
#     All branches project to aspp_ch features, then fused → in_ch.
#     """
...
#         n_branches    = 1 + len(dilations) + 1   # 1x1 + dilated + pool
...
# class UNet(nn.Module):
#     """
#     Standard UNet encoder-decoder with:
#       • ASPP at the bottleneck  (replaces the flat Down→Up bridge)
#       • Deeper classification head with residual 3×3 refiner
#       • Output: (B, NUM_CHANNELS, H, W)  raw logits
...
#         # ── Bottleneck: ASPP ─────────────────────────────────────────────
#         bottleneck_ch = f * 16 // factor            # 512 (bilinear) or 1024
#         # ASPP dilations: smaller on high-resolution bottleneck maps is better;
#         # at 512×512 input, the bottleneck is 32×32 — r=2,4,6 is appropriate.
#         self.aspp = ASPP(bottleneck_ch, aspp_ch=bottleneck_ch // 2,
...
#         # ── Classification head ──────────────────────────────────────────
#         # 3×3 refiner: helps structure boundaries before final 1×1 projection.
#         self.head = nn.Sequential(
#             nn.Conv2d(f, f, kernel_size=3, padding=1, bias=False),
#             nn.BatchNorm2d(f),
#             nn.ReLU(inplace=True),
...
#             nn.ReLU(inplace=True),
#             nn.Conv2d(f, Config.NUM_CHANNELS, kernel_size=1),
#             # No activation — raw logits; sigmoid applied in loss / inference
#         )
...
#         -------
#         (B, NUM_CHANNELS, H, W)  raw logits
#             channels 0–12: 13 structures
#             channel   13:  skin line
#         """
...
#         x5 = self.aspp(x5)        # same shape, richer multi-scale features

#         d  = self.up1(x5, x4)
#         d  = self.up2(d,  x3)
#         d  = self.up3(d,  x2)
#         d  = self.up4(d,  x1)
#         return self.head(d)       # (B, NUM_CHANNELS, H, W)
...
#     out   = model(dummy)
#     print(f"Output : {out.shape}")    # (2, 14, 512, 512)
