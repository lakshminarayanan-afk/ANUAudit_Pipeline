...
output_dir = Path(r"C:\Users\laksh\MLN\ANUAudit_Pipeline\TEMP_OUTPUT\MODALITY")
...
checkpoint = torch.load(r'C:\Users\laksh\MLN\ANUAudit_Pipeline\weights\modality\modality_model.pth', map_location=device)
...
image_paths = get_image_paths(INPUT_MODE, folder=folder, df=df if INPUT_MODE == 'csv' else None)
...
for img_path in tqdm(image_paths, desc='Unified Inference'):
    ...
    # gt = gt_lookup.get(img_path, {})

    # gt_anatomy = gt.get("anatomy", "")
    # gt_plane = gt.get("plane", "")
