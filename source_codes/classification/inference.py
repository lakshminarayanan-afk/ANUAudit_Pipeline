...
def infer(model, img_paths, root_dir, output_root, num_classes, device, num_gradcam=5, IMG_SIZE=None):
    results = []
    inference_times = []
    ...
    if device.type == "cuda":
        ...
        # Use the first image as a warmup input
        ...
        # Make sure all CUDA work is finished
        torch.cuda.synchronize()

        print("GPU warmup complete.")
        
    for idx, img_path in enumerate(img_paths):
        input_tensor, rgb_img = preprocess_image(img_path, device, IMG_SIZE)
        ...
        with torch.no_grad():
            ...
            pred_anat, pred_plane, pred_plane_raw, top_indices, top_probs = \
        ...
        print(f"INference rime of image:{inference_time_ms}")
        inference_times.append(inference_time_ms)


        # Top 3
        top1_class = idx2plane[top_indices[0, 0].item()]
        top1_prob = top_probs[0, 0].item()

        top2_class = idx2plane[top_indices[0, 1].item()]
        top2_prob = top_probs[0, 1].item()

        top3_class = idx2plane[top_indices[0, 2].item()]
        top3_prob = top_probs[0, 2].item()

        anatomy_class = idx2anatomy[pred_anat.item()]
        ...
        # Final prediction
        pred_plane = idx2plane[pred_plane.item()]

        results.append({
            "image_path": img_path,
            "predicted_model_class": pred_plane,
